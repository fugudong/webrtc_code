...
SERVER_PATH='/data/rtc/room_server/'
CONFIG_FILE = '/data/rtc/room_server/room_server_monitor.ini'
LOG_FILE_PATH = '/data/rtc/room_server/monitor_log/'
INTERVAL = 5
...
def processInstance():
    ret = os.popen('ps -ef |grep "python.*room_server_monitor.py" |grep -v grep |grep -v sudo |wc -l').read().strip()
    if ret != str(1):
...
def sendMail(cfg, process, content):
        ...
        msg['Subject']="实时音视频房间服务器报警信息" #邮件的主题，也可以说是标题
...
def getLogger():
    logger = logging.getLogger('room_server_monitor')
    ...
    logFileName = LOG_FILE_PATH + 'room_server_monitor.log'
...
if __name__ == '__main__':
    ...
    sendMail(cfg, 'room_server_monitor', 'room_server_monitor start...')
    #print stateDict
    while True:
        for process in cfg.sections():
